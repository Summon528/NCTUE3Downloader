...
class OldE3:
    ...
    async def sub_session_login(self, session: ASPSession) -> None:
        await session.get(self._url + "/login.aspx")
        resp = await session.post(
            self._url + "/login.aspx",
            data={
    ...
    async def login(self, username: str, password: str) -> bool:
        ...
        resp = await self._logged_in_session.post(
            self._url + "/login.aspx",
            data={
                "txtLoginId": username,
    ...
    async def _to_course_page(self, session: ASPSession, course: Course) -> None:
        await session.post(
            self._url + "/enter_course_index.aspx",
            data={"__EVENTTARGET": course.course_id},
        )

    def _parse_folder_table(
        self, soup: BeautifulSoup, folder_property: FolderProperty, course: Course
    ) -> Iterable[E3File]:
            ...
            for file in files:
                ...
                if matched:
                    yield E3File(
                        ...
                        int(datetime.strptime(td_els[2].text, "%Y/%m/%d").timestamp()),
                    )

    async def _get_materials(
        self, session: ASPSession, course: Course
    ) -> AsyncIterable[E3File]:
        ...
        for folder_property in self._folder_properties:
            data_navigator_text = soup.find(
                id=folder_property.data_navigator_el_id
            ).text
            ...
            if matched:
                ...
                for _ in range(total_page - 1):
                    for file in await self._get_remaining_materials(
                        session, folder_property, course
                    ):
                        yield file

    async def _get_remaining_materials(
        self, session: ASPSession, folder_property: FolderProperty, course: Course
    ) -> Iterable[E3File]:
        resp = await session.post(
            self._url + "/stu_materials_document_list.aspx?Anthem_CallBack=true",
            data={
        ...
        for table_el_id in [
            ...
            if table_el:
                ...
                for el in table_el("tr")[1:]:
                    td_els = el("td")
                    assign_id = re.search(
                        r"crsHwkId=([^;,']*)",  # type: ignore
                        td_els[-1]("a")[0]["onclick"],
                    ).group(1)
                    futures.append(self._get_assign_detail(session, assign_id, course))
                for future in asyncio.as_completed(futures):
                    files = await future
                    for file in files:
                        yield file

    async def _get_assign_detail(
        self, session: ASPSession, assign_id: str, course: Course
    ) -> Iterable[E3File]:
        resp = await session.post(
            self._url + "/ajaxpro/WebPageBase,App_Code.ashx",
            headers={"X-AjaxPro-Method": "setToken"},
        )
        token = json.loads(await resp.text())["value"]
        soup = await session.get(
            self._url
        ...
        def gen():
            if files:
                for file in files("a"):
                    file_id = re.search(
                        r"AttachMediaId=([^;,'&]*)", file["href"]
                    ).group(1)
                    file_url = self._url + file["href"].replace(
    ...
    async def _get_course_all_files(
        self, course: Course, session: ASPSession = None
    ) -> AsyncIterable[E3File]:
        if not session:
            session = ASPSession()
