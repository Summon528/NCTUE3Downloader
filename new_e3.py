...
class NewE3:
    ...
    async def _get_token(self, username: str, password: str) -> str:
        resp = await self._session.post(
            "https://e3new.nctu.edu.tw/login/token.php",
            data={
                "username": username,
                "password": password,
    ...
    async def _get_userid(self, username: str, token: str) -> str:
        resp = await self._session.post(
            self._api_url,
            data={
    ...
    async def __get_course_list(self, userid: str, token: str) -> Iterable[Course]:
        resp = await self._session.post(
            self._api_url,
            data={
        ...
        return (
            Course(str(course["id"]), self._scrub_course_name(course["fullname"]))
            for course in resp_json
            if course["enddate"] > time.time()
        )

    async def _get_folders(
        self, token: str, courses: Iterable[Course]
    ) -> Iterable[Folder]:
        payload = {"wsfunction": "mod_folder_get_folders_by_courses", "wstoken": token}
        ...
        return (
            Folder(
                ...
                if folder["name"].startswith("[Reference]")
                or folder["name"].startswith("[參考資料]")
    ...
    async def _get_materials(
        self, token: str, course_name: str, folder: Folder
    ) -> Iterable[E3File]:
        resp = await self._session.post(
            self._api_url,
            data={
        ...
        resp_json = json.loads(await resp.text())
        files = [x["modules"][0] for x in resp_json if len(x["modules"])][0]["contents"]
        return (
            E3File(
                file["filename"],
                course_name,
    ...
    async def _get_assign_files(
        self, token: str, course_dict: Dict[str, str], courses: Iterable[Course]
    ) -> Iterable[E3File]:
        payload = {"wsfunction": "mod_assign_get_assignments", "wstoken": token}
        for idx, (courseid, _) in enumerate(courses):
            payload[f"courseids[{idx}]"] = courseid
        resp = await self._session.post(self._api_url, data=payload)
        resp_json = json.loads(await resp.text())
    ...
    async def all_files(self) -> AsyncIterable[E3File]:
        ...
        futures = [
            self._get_materials(self.token, course_dict[i.course_id], i)
